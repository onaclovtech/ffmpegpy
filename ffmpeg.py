...
class ffmpeg:
    ...
    def extractAudio(self,output_filename_and_path):
      ...
      try:
         print 'ffmpeg -i ' + self.filename + '-q:a 0 -map a ' + output_filename_and_path
    ...
    def combineFrames(self, output_filename_and_path):
      print 'ffmpeg -framerate ' + self.framerate + ' -i ' + self.path + '/Pictures%d.jpg' + ' -i ' + self.audio_file + ' -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p ' + output_filename_and_path
      try:
         subprocess.check_output(['ffmpeg -framerate ' + self.framerate + ' -i ' + self.path + '/Pictures%d.jpg' + ' -i ' +
                                self.audio_file + ' -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p ' + output_filename_and_path], stderr=subprocess.STDOUT,shell=True)
      except subprocess.CalledProcessError as e:
            print 'combineFrames'
            print e.output
